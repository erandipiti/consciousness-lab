...
| `ManifestStream` (as a list) | per-stream summary block | **replaced by `stream_close_status` map** | closure status is the only stream fact with no other on-disk home; a map cannot carry duplicate ids | duplicate `stream_id` detection |
...
| `record_sha256` in sealed logs (chunks, lifecycle, events) | per-record identity | **removed** | derivable from the record's canonical bytes, and the next record's `prev_record_sha256` is that value. Kept in `annotations.jsonl`, which is outside the manifest DAG and whose head pointer names it | record self-hash ↔ record bytes, in three logs |
...
| Persisted copies of a record's own hash | 1 per record, 4 logs | 1 log only (`annotations.jsonl`) |
...
**Relations removed: 32** — 26 from the first draft, plus 6 more from the
authority/minimality design review (`payload_ref`'s four, per-record self-hashes,
and manifest identity). Two are **added** deliberately — Arrow schema
...
| `record_sha256` in `annotations.jsonl` | load-bearing — that file is outside the manifest DAG and `annotations.head.json` names the value |
